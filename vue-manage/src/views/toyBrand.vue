<template>
  <div >
    ...
    <!-- 新增 修改表单 -->
    <el-dialog title="玩具品牌信息" :visible.sync="formVisible" width="30%" @close="cancel" :close-on-click-modal="false">
      <el-form :model="formData" ref="formRef" :rules="rules">
                <el-form-item prop="brandName" label="品牌名称" label-width="120px">
          <el-input v-model="formData.brandName" autocomplete="off" style="width: 80%"></el-input>
        </el-form-item>
        <el-form-item prop="description" label="描述" label-width="120px">
          <el-input v-model="formData.description" autocomplete="off" style="width: 80%"></el-input>
        </el-form-item>
        <el-form-item prop="createTime" label="创建时间" label-width="120px">
          <el-date-picker style="width: 80%" v-model="formData.createTime" type="datetime" value-format="yyyy-MM-dd HH:mm:ss" placeholder="选择日期时间"></el-date-picker>
        </el-form-item>
  ...
  methods:{
    ...
    del(id){
      this.$confirm('此操作将永久删除信息, 是否继续?', '提示', {
        ...
        request.delete(`/toyBrand/${id}`).then(res=>{
          if(res.code==='200'){
    ...
    delBatch() {
      ...
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        request.delete('/toyBrand/deleteBatch',{data:this.idList}).then(res=>{
          if(res.code==='200'){
            this.$message.success('操作成功')
            this.load()
          }else{
