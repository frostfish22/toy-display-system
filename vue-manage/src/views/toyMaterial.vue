<template>
  <div >
    ...
    <!-- 新增 修改表单 -->
    <el-dialog title="玩具材质信息" :visible.sync="formVisible" width="30%" @close="cancel" :close-on-click-modal="false">
      <el-form :model="formData" ref="formRef" :rules="rules">
                <el-form-item prop="material" label="材质" label-width="120px">
          <el-input v-model="formData.material" autocomplete="off" style="width: 80%"></el-input>
        </el-form-item>
  ...
  methods:{
    ...
    del(id){
      this.$confirm('此操作将永久删除信息, 是否继续?', '提示', {
        ...
        request.delete(`/toyMaterial/${id}`).then(res=>{
          if(res.code==='200'){
    ...
    delBatch() {
      ...
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        request.delete('/toyMaterial/deleteBatch',{data:this.idList}).then(res=>{
          if(res.code==='200'){
            this.$message.success('操作成功')
            this.load()
          }else{
